...
<head>
    ...
    <!-- SEO метаданные -->
    <title>{{ $template->name }} | {{ config('app.name') }}</title>
    <meta name="description" content="{{ $template->description ?? 'Просмотр шаблона ' . $template->name }}">
    ...
    @if(isset($template->custom_data['external_styles']))
        @foreach($template->custom_data['external_styles'] as $style)
...
<body>
    ...
    <div class="info-panel" id="infoPanel">
        ...
        @auth
            <a href="{{ route('client.templates.editor', $template->id) }}" class="btn-use">Использовать шаблон</a>
        @else
    ...
    <div class="card-footer">
        <a href="{{ route('client.templates.create-new', $template->id) }}" class="btn btn-primary">
